...
uniform float delta;
//constants
uniform int MAX_SAMPLES;	//total samples for each ray march step
...
{ 
	...
	for (int i = 0; i < MAX_SAMPLES; i++) {
		...
		vec4 tex = texture(volume, dataPos);	
			vec4 noisevec = texture(volume,vUV + vec3(delta,0.0,0.0));
				...
				vec3 color = mix(SkyColor,CloudColor,intensity) * 2.5;
				sample = color.b;
		...
		vFragColor.rgb = prev_alpha * vec3(color) + vFragColor.rgb; 
